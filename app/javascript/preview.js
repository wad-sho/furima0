...
//     // ここで、class属性を付与しているのはcssで画像サイズを変更できるようにするため？
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  const buildNewFileField = () => {
    ...
    const newFileField = document.createElement('input');
    newFileField.setAttribute('type', 'file');
    newFileField.setAttribute('name', 'items_tag[images][]');

    // 最後のfile_fieldを取得
    const lastFileField = document.querySelector('input[type="file"][name="items_tag[images][]"]:last-child')
  ...
  // input要素を取得
  const fileField = document.querySelector('input[type="file"][name="items_tag[images][]"]');
